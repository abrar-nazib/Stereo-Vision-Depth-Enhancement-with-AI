...
model = torch.hub.load("pytorch/vision", "raft_stereo", pretrained=True).to(device)
model.eval()

# Image preprocessing for RAFT-Stereo
...
while True:
    ...
    if retL and retR:
        # Convert to grayscale (for visualization purposes only)
        ...
        h, w = rect_L_color.shape[:2]
        rectL_tensor = transform(rect_L_color).unsqueeze(0).to(device)
        rectR_tensor = transform(rect_R_color).unsqueeze(0).to(device)

        # Compute disparity with RAFT-Stereo
        with torch.no_grad():
            disparity = model(rectL_tensor, rectR_tensor)
            disparity = disparity.squeeze(0).cpu().numpy()  # [H, W]

        # Resize disparity to match image dimensions if necessary
        ...
        print("Failed to capture frames. Attempting to reconnect cameras...")
        CamL.release()
        CamR.release()
        if os.name == "nt":
            CamL = cv2.VideoCapture(CamL_id, cv2.CAP_DSHOW)
